 ...
 *
 * The SubMenu offers this behavior via the trigger() method, into which both the task
 * to be executed and the waiting period (delay) must be passed in as arguments. Following
 * the example:
 *
 * 		const SubMenu = new SubMenu(WAITING_PERIOD);
 * 		const letters = [];
 *
 * 		function letterReceived(l) {
 * 			letters.push(l);
 * 			SubMenu.trigger(() => { return makeTheTrip(); });
 * 		}
 */
export class SubMenu {
	...
	void trigger(task ITask, delay defaultDelay) (ref auto SubMenu) @local {
		this.task = task;
		this.acceptTimeout();
